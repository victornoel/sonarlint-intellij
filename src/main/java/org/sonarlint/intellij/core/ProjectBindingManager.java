...
public class ProjectBindingManager {
  ...
  public ConnectedAnalysisEnv getConnectedAnalysisEnv() {
    if (getSettingsFor(myProject).isBound()) {
      return tryGetServerConnection()
        .map(connection -> new ConnectedAnalysisEnv(getConnectedEngineSkipChecks(), connection))
        .orElse(null);
    }
    return null;
  }
  ...
  public void unbind() {
    var previousBinding = getAnalysisEnv();
    var projectSettings = getSettingsFor(myProject);
    ...
    var builtInBinding = getStandaloneAnalysisEnv();
    if (!previousBinding.equals(builtInBinding)) {
      myProject.getMessageBus().syncPublisher(AnalysisEnvListener.TOPIC).analysisEnvChanged(previousBinding, builtInBinding);
    }
  }
